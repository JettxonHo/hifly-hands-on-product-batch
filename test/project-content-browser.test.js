...
test("system Chrome completes and restores the project-content flow", async (t) => {
  ...
  await page.locator("#newPassword").fill("Browser-Content-Password-9!"); await page.getByRole("button", { name: "保存并进入工作台" }).click(); await page.waitForURL(`${origin}/`);
  await page.getByText("项目", { exact: true }).click(); await page.getByLabel("项目名称").fill("八月新品"); await page.getByRole("button", { name: "创建项目" }).click(); await page.getByText("打开", { exact: true }).click();
  await page.getByLabel("商品名称", { exact: true }).first().fill("云朵抱枕"); await page.getByRole("button", { name: "创建商品" }).click();
  ...
  await page.getByRole("button", { name: "设为 Ready" }).click(); await page.getByText("商品快照已 Ready。").waitFor(); assert.match(await page.locator("#revisionState").textContent(), /^ready/);
  await page.reload(); await page.locator("#revisionState").waitFor(); assert.match(await page.locator("#revisionState").textContent(), /^ready/);
