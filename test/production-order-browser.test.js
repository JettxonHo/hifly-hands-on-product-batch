...
test("production workspace supports empty/create/re-entry flows at desktop and 390px without A10 buttons", async (t) => {
  ...
  await page.route("**/api/cloud-executor/status", (route) => route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(cloudState) }));
  ...
  await page.reload();
  await page.getByText("Cloud Executor 正在执行", { exact: true }).waitFor();
