(async () => {
  ...
  function render(projects) {
    ...
    for (const project of projects) {
      ...
      const detail = document.createElement("div"); const name = document.createElement("strong"); name.className = "row-title"; name.textContent = project.name; name.title = project.name;
  ...
  form.addEventListener("submit", async (event) => { event.preventDefault(); formError.textContent = ""; const data = new FormData(form); const button = form.querySelector("button"); button.disabled = true;
    try { await request("/api/projects", { method: "POST", headers: { "content-type": "application/json", "idempotency-key": crypto.randomUUID() }, body: JSON.stringify(Object.fromEntries(data)) }); form.reset(); dialog.close(); await refresh(); }
  ...
  document.querySelector("#openProjectDialog").addEventListener("click", () => { formError.textContent = ""; dialog.showModal(); form.name.focus(); });
