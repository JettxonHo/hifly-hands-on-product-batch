...
- production PostgreSQL 通过专用 read port 在单个 `REPEATABLE READ` 事务内完成 filter/total/anchor/page，只锁定和读取当前页最多 6 个 Work，也只为当前页缺 inspection 的 Work 初始化 pending inspection/audit/ledger；页外 Work 保持零写。paged GET 因页内 lazy 初始化仍不得称为纯读 BFF；没有新增 DB、migration、领域状态或写命令。
