...
## 2026-09-02 Issue #278 follow-up: Production Evidence Contract correction

- Follow-up work keeps `0ac54ff` immutable and continues on
...
- Cloud and Local real V1 paths use the same bounded structured pre-paid
  ...
  thrown evidence. Local reports persist sanitized evidence via existing
  `supporting_outputs` JSON and preserve `failure_stage`.
- Hifly generated-result dimensions are read only from a single visible,
...
- Focused/latest checks: helper 8/8, contract 20/20, Cloud Playwright 13/13,
  ...
  terminated; it is not treated as a product failure. Continue with
  independent Review/exact-head CI; do not merge/push/deploy or run real
  production without a new Owner Gate.
...
- 当前执行顺序为：REL-001 PR #256 已由 Owner 授权管理员 squash 合并；更新并复审公共数字人缩略图 PR #253；冻结并部署不含域名/TLS 上线的内部发布版本；完成运营人员无积分流程验收；最后仅在 Owner 另行明确授权积分后执行一个新商品的真实生产验收。
...
- PR #253 公共人物缩略图同步仍是独立 Ready PR，不属于 Issue #254。视觉升级 merge commit 为
...
- production PostgreSQL 通过专用 read port 在单个 `REPEATABLE READ` 事务内完成 filter/total/anchor/page，只锁定和读取当前页最多 6 个 Work，也只为当前页缺 inspection 的 Work 初始化 pending inspection/audit/ledger；页外 Work 保持零写。两个首次 GET 竞争初始化同一 Work 时，败者只会走受控 projection-invalid 恢复，不泄漏 raw PostgreSQL unique code，也不会产生第二组初始化记录。paged GET 因页内 lazy 初始化仍不得称为纯读 BFF；没有新增 DB、migration、领域状态或写命令。
- 页面保留 passed 不等于 delivered、download 不等于 delivery；最新 rework 状态优先于历史 delivery，同时保留完整交付历史。读取失败清除旧 detail/action。模糊网络/408/5xx 只允许显式载入权威状态，并以原 idempotency key 精确重放确认 receipt；不得凭状态或交付数量推断本次成功。不确定 intent 未收口前阻止作品切换、分页、筛选和 same-document Back，避免原 key 被另一上下文覆盖；确定性 409 才换用最新 inspection binding 与新 key。390/768 每态最多一个可见可执行主操作，不可用/已撤回 Work 零 Dialog、零 POST。下载 token 精确绑定 Work、AssetVersion 和已核验字节，跨 Work URL 不能消费。
...
- 唯一真实命令完成 heartbeat、claim、start、交接包下载、人物与商品上传，并点击一次飞影手持商品图生成。飞影等待约 30 分钟后明确显示“生成失败”；没有提交外层视频生成。
- attempt `2aaea76b-d770-4e5e-bc89-a486a2b7cb79`、报告 `3b33138b-40dd-4af4-b708-291a3e5e23a3` 已落盘；工单和 attempt 为 failed，报告为 `failed / not_retryable`，candidate 0、Work 0。失败后停止，没有自动重试。
- 旧状态识别会把失败弹窗中的“再次生成 / 重新编辑 / 确认”误判为成功操作组，随后误报 AI 文案开关未关闭。现已将“生成失败”设为优先失败信号：等待器立即以安全错误退出，成功确认和文案/视频阶段均不会继续。
- 本次点击了飞影 `立即生成 150积分`，实际扣费以飞影后台账单为准；失败截图显示余额 `56191`，该读数可能包含上一轮延迟结算，不能据此计算本轮精确扣分。
- 本次真实授权已经使用。当前报告不可重试；若继续真实验收，必须先合并本轮失败态修复，再创建新的 reproduction 工单并由 Owner 重新授权最多 1 条真实生成。
- 详细记录：`docs/status/sessions/2026-08-10-fifth-real-local-agent-hifly-generation-failure.md`。

## 2026-08-10 第四次真实 Local Agent 已生成飞影作品；下载根目录错误失败即停并完成无积分修复

- Owner 新授权最多执行 1 条真实飞影生成。通过正式 API 创建 reproduction 工单 `bc0153e2-1f2c-49bd-a75d-ab909fb28a20`，交接包 `e4ed7df0-0b87-4ce4-acfd-17948215dff9` 为 `ready / v1`；执行前确认其为唯一待执行工单、attempt 数为 0，飞影预检为 ready。
- 唯一真实命令成功完成商品图与人物上传、手持商品图生成确认、批准文案写入和外层视频提交。飞影作品 `692503`（页面时间 `2026-08-10 14:20:20`）生成完成，脚本正确点击其下载按钮。
- attempt `f71a9bfd-51e2-4312-bc5d-206dd09c6504` 在下载文件落盘前失败；报告 `ed678819-b5f6-4bcb-b023-816492523325` 已落盘，工单为 failed、candidate 0、云端无 Work。失败后停止，没有自动重试。
- 根因是 Local Agent 的系统临时下载目录与 Hifly 适配器配置根目录不同：下载按钮点击后，适配器在 `saveAs` 前把合法临时路径当成越界路径。批处理现通过内部执行上下文显式传递本次 `projectRoot`，Hifly 下载器据此校验并返回相对路径；既有仓库外任意路径拒绝测试仍保留。
- 本次确实点击了飞影手持商品图及外层视频生成，积分会以飞影账单实际结算；页面提交时仍显示旧余额 `56841`，不能据此推断最终扣除数。
- 本次真实授权已经使用。当前失败报告为 `not_retryable`，修复合并后仍需创建新的 reproduction 工单并重新取得 Owner 单条积分授权，禁止直接重复 real 命令。
- 详细记录：`docs/status/sessions/2026-08-10-fourth-real-local-agent-download-root-failure.md`。
...
- Sol 最终 Review 无 Blocker/Important；实现提交 `605b82d` 已推送并创建 ready PR #97，Ubuntu、Windows、PostgreSQL CI 全绿。下一步仅等待 Owner 合并决定；未经新授权不合并。当前关键飞影批次未触碰，积分消耗为 0。
...
- 当前 `asset_evidence` 结构为 `{asset_id}`（非 URL id），GUI 不消费（`web/` 无引用），风险低；隐藏是为彻底收敛 evidence public 面 + 防御未来 `createAsset` 返回更多字段。
- 至此 evidence 类 public 面已彻底收敛：`remote_evidence`/`remote_candidates`/`submit_checkpoint` 经投影 + URL 过滤，`asset_evidence` 隐藏；`execution_snapshot.items` 同走 `publicItem`；retry 清理无残留。
- 新增回归测试（`test/server-api.test.js`）：seed `asset_evidence` 含 `upload_url`/`remote_url`，断言 public item 不含 `asset_evidence`、不含 cdn/token。
- 验证：`npm run check` 65 文件；`npm test` 391/391；`node --test test/server-capture-api.test.js` 30/30；`node --test test/server-api.test.js` 46/46；`node --test test/gui-smoke.test.js` 12/12；`git diff --check main...HEAD` clean。全程**未访问飞影、未跑真实 HTTP、未消耗积分**。
...
- 代码复核后补了一处真实抓包形态的稳定性修复：录制 manifest 的 mock 响应经常包含固定 `remote_id` / 文件名；`mock` 模式现在会按 `task_id` 生成唯一 artifact id 与文件名，避免小批量第二条开始因同名产物失败。`real_dry_run` 与 `real_live` 文件语义保持不变。
...
- 曾尝试调用本机 Claude Code 作为开发 subagent；Claude CLI 健康检查可用，但长任务执行卡住且只留下一个无效 import，已撤销该无效 diff。后续短 review 尝试因预算限制退出；最终实现和复核由 Codex 主控完成。
- 本轮未访问飞影、未跑 Playwright 真实生成、未跑真实 HTTP、未生成视频、未消耗新增积分。
- 已执行定向验证：`node --test test/capture-workflow-state.test.js test/server-capture-api.test.js test/gui-smoke.test.js` 为 30/30 通过；复核补丁后 `node --test test/capture-http-executor.test.js test/server-capture-api.test.js` 为 28/28 通过。
- 已执行最终验证：`npm run check` 通过（65 个 JS 文件）；`git diff --check` 通过；`npm test` 为 368/368 通过。
...
- `real_live_completed` 会显示“抓包 HTTP：已完成”、飞影作品 ID、下载路径、完成时间，并提供“复制路径”按钮。
- `real_live_failed` 会显示“抓包 HTTP：失败，可到待执行任务中重新真实 HTTP 生成”、稳定错误码和错误信息。
- 其他中间抓包状态会显示“抓包工作流：<状态>”，方便运营从运行记录判断该批次是否只是录制/预演而非 HTTP 出片。
- 本轮未访问飞影、未跑真实 HTTP、未生成视频、未消耗新增积分；仅做本地 GUI 展示修改。
- 已执行定向验证：`node --test test/gui-smoke.test.js` 为 9/9 通过。
- 下一步建议：P1 剩余可做“安全下载/打开产物按钮”的后端 API 设计；注意 public batch 目前只公开 artifact id，不公开 artifact path，不能用前端拼路径绕过 artifact 安全检查。

## 2026-07-19 剩余 TODO 持久化：按 P0-P3 继续推进

当前可用结论：

- 仅靠 Playwright 已经实现当前项目核心业务闭环：GUI 商品录入/导入 → 一商品一条片 → 飞影“手里有货”页面自动上传、生成手持图、确认、提交视频、等待作品、下载产物、GUI 状态/失败重试。
- 抓包 HTTP 已完成单条真实出片联调，但还不是批量生产默认路径。当前生产建议仍是 Playwright 批量；抓包 HTTP 只作为单条授权联调和下一代替代路径。
- 本地工作区没有新的项目相关 WIP 代码需要接力；仍存在既有无关改动 `.superpowers/sdd/task-6-report.md` 与未跟踪 `docs/resume/`，不要误提交，除非用户明确要求处理。

P0 必须保持：

- 继续保护 Playwright 批量生产链路，任何抓包 HTTP 改动都不能改变默认生产路径。
- 真实访问飞影、真实 HTTP 出片、Playwright 真实批量生产前，必须获得用户明确授权并记录是否可能消耗积分。
- GUI 必须保留失败/异常批次恢复入口：`failed`、`interrupted_unknown`、抓包 `real_live_failed` 均不能要求用户重新逐条录入。
- 所有重要进度继续写入 `docs/PROJECT_HANDOFF.md`；生产流程变更同步 `docs/SOP.md`。

P1 近期收尾：

- 运行记录集中展示抓包 HTTP 摘要：例如 `real_live_completed` 显示飞影作品 ID、下载路径、完成时间；`real_live_failed` 显示错误码和下一步动作。
- 更清晰地区分运行记录中的 Playwright 普通产物与抓包 HTTP 单条联调产物。
- 继续改善产物管理：目前已有“复制路径”；下一步可谨慎设计“下载/打开产物”按钮。注意后端 public batch 目前只公开 artifact id，不公开 artifact path，不能绕开安全边界直接拼本地路径。
- SOP/新人手册继续同步 GUI 当前状态，尤其是“同时录制抓包产物（不切换批量后端）”的含义。

P2 中期设计与实现：

- 设计抓包 HTTP 小批量队列，但默认不启用：逐条执行、限速、失败停机、失败续跑、重复提交保护、下载校验、积分预算/上限提示。
- 扩展错误分类和 GUI 提示：登录态失效、OSS 上传失败、飞影业务拒绝、作品 URL 未 ready、下载失败、接口结构变化。
- 为 HTTP 小批量新增无网络/fake transport 测试，再由用户明确授权后只跑小样本真实联调。

P3 后续增强：

- 评估抓包 HTTP 是否能替代 Playwright 成为默认批量后端；只有 HTTP 小批量稳定后再考虑。
- 多账号/多浏览器 profile、跨平台打包、自动更新、客户侧轻量发行。
- 人物池/背景池推荐策略、视频画面重复度质检、姿势/商品识别质检。

下一步建议：

1. 优先做 P1“运行记录抓包 HTTP 摘要集中展示”，这是无积分 GUI 改动。
2. 之后再评估 artifact 下载/打开按钮的安全 API 设计。
3. 暂不进入 P2 HTTP 小批量真实执行，除非用户明确要求并授权可能消耗积分。

## 2026-07-19 P0/P1 第一轮收尾：路径复制与抓包录制边界文案（无新增积分）

- GUI 已在任务输出文件和真实 HTTP 成功摘要旁增加“复制路径”按钮，方便运营从批次详情或运行记录中复制本地产物路径，不需要手动选中文本。
- 三个创建入口的抓包选项文案已统一为“同时录制抓包产物（不切换批量后端）”，明确该选项只保存 HAR，不会把 Playwright 批量生产切换为抓包 HTTP。
- SOP 已同步当前推荐生产路径：批量生产默认继续使用 Playwright；抓包 HTTP 已跑通单条真实出片，但仍只作为单条授权联调入口，不建议直接用于多商品批量。
- 本轮未访问飞影、未跑真实 HTTP、未生成视频、未消耗新增积分；仅做本地 GUI 和文档修改。
- 已执行定向验证：`node --test test/gui-smoke.test.js` 为 9/9 通过。
- 下一步建议：继续 P0/P1 剩余项时，可优先做运行记录中的抓包 HTTP 摘要集中展示，或者谨慎设计 artifact id 到下载 URL 的安全映射，再考虑“打开/下载产物”按钮。
