...
- `provider_reference_observed_at`, `generation_context_version`；
- `created_at`。
...
完整 Provider URL、Cookie、Token、浏览器 Profile 路径、请求头和签名参数不得持久化。公共 API 不返回原始
`provider_reference`，只返回可用状态、类型和观察时间；内部 handoff/adapter 只取得完成视频所需的最小受控引用。

候选不可变，当前可用性由服务端投影 `candidate_state` 表达：

| 状态 | 含义 | 可进入检查/审核/Production |
|---|---|---|
| `available` | 候选 AssetVersion 可读、上游仍 current、Provider 引用当前可用 | 可继续 |
...
读取失败或状态无法确定时，API 返回失败而不是把未知映射为 `available`。只有确认对象缺失或完整性不一致后才写入
`candidate_unavailable`；临时读取错误保持 fail-visible，不静默改写历史。

### 4.3 `AppearanceCheckRun` 与 `AppearanceCheckResult`
...
`passed`。模型、阈值、供应商和费用不在 Fidelity-A 决定，后续能力 gate 未通过前不得实现假检查器。

### 4.4 `AppearanceReview`
...
以下变化使当前候选 `upstream_changed`，同时撤销当前 approved AppearanceReview，但保留 candidate/check/review 历史：
...
Provider 引用无法继续视频时使用 `reference_unavailable`，不篡改候选 bytes，也不把它误写成外观检查失败。恢复引用不能
自动恢复审核；由于长期恢复能力未知，当前唯一安全动作是新建 capture request、取得新单条授权并生成新候选。
...
- exact current `AppearanceCandidate` 属于同一 Organization/Product/ProductRevision；
...
- 当前 check result 可用于批准，且没有 unresolved/unknown/blocked/failed；
- exact AppearanceReview 为 current `approved`，未 revoked/superseded；
- Provider reference 当前为 `available`，不是 unknown 或 unavailable。
...
| `GET /api/appearance-candidates/:candidateId` | `{candidate}` | 精确候选；404 不泄漏跨组织存在性 |
...
公共 candidate 投影包括上游 ID、candidate AssetVersion ID、核验元数据、状态、检查/审核摘要和审计时间；不包括 object key、
Provider URL、原始 Provider reference 或凭据。
...
Provider reference 的 `available` 只能来自 Fidelity-B 已接受的无副作用 read/recovery observation，并保存
`observed_at`。如果观察需要新的付费候选、会改变 Provider 状态或 seam 已漂移，状态必须保持 unknown 并阻断，而不是由
时间、`gen_id` 非空或历史成功推断。
...
检查启动命令只接受 candidate ID、`expected_candidate_revision` 和幂等键，不允许客户端选择未获批准的模型、阈值或供应商。
如后续检查能力本身收费，必须先经过独立费用授权设计，不得复用候选授权。

批准 body 包含 `expected_revision`、精确 `appearance_check_result_id`，以及 needs_review 时逐维的
`resolved_unknowns:[{dimension,decision_reason,evidence_reference}]`。blocked/failed、候选不可读、上游失效或仍有 unresolved
dimension 时返回 422。拒绝要求受控 reason；批准和拒绝均幂等且写审计事件。
...
`candidate_unreadable`、`candidate_upstream_changed`、`provider_reference_unavailable`、`check_missing`、`check_failed`、
...
- candidate AssetVersion 服务端核验完成、AppearanceCandidate 创建、request succeeded 和事件在同一事务，或使用可重放的
  outbox/补偿流程，绝不能出现 request succeeded 但候选 bytes 不可读；
- 同一 product/upstream fingerprint 同时最多一个 queued/running capture request；冲突返回 409，不静默合并；
- check result 与 findings 不可变；同一 candidate/policy version 最多一个 active run；
- review transition 使用 expected revision；批准前在事务内重读 candidate/check/upstream；
- ProductionOrder 创建在事务内重读 exact approved review 和 current evidence chain；
- 所有系统自动失效使用 row_version，冲突后重读并保持已失效结果幂等；
...
| Provider reference 失效 | candidate 保留审计，review revoked；取消被阻断的 waiting 工单，新 request + 新授权生成新候选 |
...
1. 新建 appearance capture/candidate/check/review/audit/receipt 表或等价深模块；不改写现有历史对象；
...
- 实现 request/authorization、精确源图读取与 upload proof、候选 bytes 写入 AssetVersion、Provider reference、失败关闭；
...
- GREEN：一次授权最多一个 candidate、bytes 可读、状态/审计准确；
- 停止：候选无法在视频前保存并结束任务，或需要长期持有 Production lease。

### Fidelity-C：自动检查与运营审核

- 实现 check run/result/findings、candidate workspace、review/approve/reject/revoke、冲突与历史；
...
- 扩展 create/claim gate、snapshot、handoff 和 adapter 的显式 candidate/provider reference 输入；
...
- 停止：无法证明一次 approved candidate 只对应一次视频提交。
...
- Provider 引用长期/跨设备有效性未知，因此本合同不承诺隔日或跨节点恢复；
