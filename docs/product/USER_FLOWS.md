...
- **主路径**：
  ...
  2. 生成多版文案草稿，系统运行**确定性规则质检 + LLM 语义质检**，给出结构化 finding（命中片段、原因、证据、规则来源、修复建议），并产出自动质检结果 `invalid` / `blocked` / `needs_review` / `passed`（D-025）；
  3. 运营修改、局部重写或请求 AI 改写（只改开头/某句/口语化/平台风格）——任一正文变化**创建新 CopyVariant/版本并完整重新质检**，对比版本后固定；
  4. `passed` 文案提交人工审核；审核人逐项处理 `needs_review` finding（记录接受理由）、要求修改或驳回，最终 `approved` / `changes_requested`；
