...
- **角色**：电商运营
- **前置条件**：已进入工作台（如当前版本启用账号体系，则已完成登录；第一版是否云端登录见 [OPEN_QUESTIONS.md](OPEN_QUESTIONS.md) Q-002，未决定前不假设云端账号体系存在）；有至少一件商品素材；Local Agent 在线且 Provider 登录态有效（或系统明确提示缺失）
- **主路径**：
