...
- owner 审查并合并产品蓝图与本决策 PR（进行中）
- 低保真页面结构（未完成）
- 第一条垂直切片落地为开发范围（Phase 1A 已由 D-017 定型，具体 Issue 拆分未完成）
- Issue 拆分（未完成）
...
目标：**第一版是 Cloud-first 的云端 Web 产品（默认部署腾讯云，单企业/单组织）；即使部分步骤仍需人工，也能完整走完产品链路。**
...
- 一个企业用户能登录云端 Web 并按五阶段走完一次生产链路（执行可依赖现有手里有货路径）；
- VideoPlan → batch 编译边界可追踪（双向 ID）；
- 看板使用运营语言，技术状态收敛到任务详情；
- 现有安全、幂等、原子写、路径与 CI 标准不降低。

### Phase 2：黄金路径与 Local Agent

目标：**打通 Phase 1A 端到端黄金路径（选择已有/公共人物）。**
...
- 云端下发 → Local Agent 执行手里有货 → 状态回传 → 作品登记闭环；
- 长 Playwright 任务不出现在 Serverless/Workers 请求生命周期；
- 云端不保存不必要的飞影 Cookie。

Phase 1B（图片数字人创建子切片）独立排期，不与 Phase 2 黄金路径塞进同一个大 PR（D-017）；真实调用前须 owner 单独授权。
