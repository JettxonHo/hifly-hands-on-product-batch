...
Issue #252 的缩略图导入已先收口为离线 source seam：真实 Provider 是否存在受权只读 thumbnail 字段/端点、是否需独立登录态、内容类型与稳定性，仍未确认；在该问题关闭前不得接真实请求或将 fixture 预览视为飞影能力证据。离线闭环只接受服务端校验后的 bytes/media/size/SHA-256，并复用既有短时 preview。
