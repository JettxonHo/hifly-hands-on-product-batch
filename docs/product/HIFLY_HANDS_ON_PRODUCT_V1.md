...
```json
{
  ...
  "production": {
    ...
    "voice_identity_policy": "ui_visible_state_allowed", "scene_mode": "single_scene",
...
Pre-paid contract mismatches (including target or voice evidence) stop before
paid action 1. The handheld ratio is observed after paid action 1 and only
blocks UI Confirm / paid action 2 when an explicit exact policy requires it.
The formal ProductionOrder snapshot currently writes `record_only`; the
`require_exact` path is a deferred execution-authorization seam, not a policy
selectable through the current order route.
...
Any pre-paid identity, mapping, media, size, checksum, mode, version, voice, or
copy-state mismatch stops before a point-consuming boundary. A post-handheld
actual-ratio mismatch is recorded after paid action 1 and is non-blocking under
the formal `record_only` snapshot; only a separately authorized `require_exact`
policy blocks Confirm/action 2. A generic verified capability never selects
this route; the explicit contract plus exact registered material
version/checksum does.
...
| 9:16 | Historical `1600x2848` artifact plus post-handheld natural-dimension seam | Formal order is `RECORD_ONLY`; deferred immutable `require_exact` seam; final-video proof still required |
...
`NEW_CODEX_RECORDING_REQUIRED=YES` for the native-voice identity fields only. Ratio target-vs-actual handling is now implemented as a post-handheld artifact gate, while a live recording is still required to establish any provider-side target normalization behavior. The semantic generated-result selector is synthetic/unverified in this engineering candidate; `LIVE_DOM_EVIDENCE_REQUIRED` remains open. A future bounded recording should capture DOM/state, selector, request/response and set/read-back values, and stop immediately before any point-consuming Generate click whenever the fields can be established without generating. No recording or live generation is performed by this engineering stage.

## P1 truth

`production.target_aspect_ratio` is the immutable intended target. The formal
ProductionOrder snapshot currently carries
`handheld_aspect_ratio_policy=record_only`; the post-handheld natural-dimension
check is **RECORD_ONLY**. An immutable
`handheld_aspect_ratio_policy=require_exact` remains a deferred explicit
execution-authorization seam and is not selectable by the current order route;
...
The historical evidence baseline keeps hands-on-product `PROVEN` and
avatar/product `PARTIAL`, while a new run without an evidence record starts all
unobserved dimensions at `NOT_PROVEN`/`NOT_CAPTURED` (including avatar, product,
and UI-visible voice). Copy/voice/final ratio and Stage 1→Stage 2 dimension
behavior remain `NOT_PROVEN` until their own evidence is observed. Product fidelity is a
...
exact provider identity; unobserved `voice_id`, `tts_voice_id`, provider
`provider_voice_name`, `display_name`, `group_id`, and submitted
`submitted_voice_name` remain `NOT_CAPTURED`/`NOT_PROVEN`. Provider name and
submitted voice name are separate fields; there is no collapsed `voice_name`.
