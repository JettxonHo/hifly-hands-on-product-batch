...
## D-038 HIFLY FAST-MVP

- **日期/来源**：2026-09-07，Owner 在当前任务完整指定 FAST-MVP 目标、R0–R6 阶段、3+3 配对方法与授权边界。
- **状态**：Confirmed direction；当前只授权隔离研发、零 Provider 验证、Review、Issue/Draft PR 和实验准备，真实上传/付费/合并/部署未授权。
- **决定**：优先工作台独立闭环，复用已有 Cloud Playwright、资产、任务、作品与安全核心。普通用户直接选人物、商品、输入中文口播和确认设置/费用，不需理解内部对象；第一轮只支持已有获权且实际可执行的人物。独立创建/训练/克隆、额外模型、发布、并发、大批量后置。
- **方法**：H0 固定为混合路径历史；正式 A 为纯 Computer Use，B 为工作台，三配对同素材/口播/可控设置，冻结代码/环境/缓存策略，A1→A2→A3→B1→B2→B3。新视频最多六条，每条两段各最多一次，不增加隐形冒烟和重试。真实运行需完整的新阶段预算与上传授权；后续十条连续验证另行批准。
- **治理**：Owner 唯一产品/预算/阶段决策者；独立代码 Review 不新增多个业务审批人。保留权限、输入绑定、费用上限、防重、持久状态、未知停止和审计；若审核流程本身需要变化，必须在具体候选中说明原/新语义，不伪造 QC/审核结果。
- **对旧合同的影响**：D-037 与其 RBV 数据/失败/未完成事实保留；旧 Goal 归档。D-038 覆盖当前研发顺序、Owner 角色和本次配对方案，旧 budget/roster 不继承，旧 RBV 非作者第二批要求不作为本轮额外人工审批门槛；不宣称旧 RBV 完成。
- **架构结论**：未预选保留、薄工作台或双层方案；R4 按真实证据再决定。重大产品/架构变化交 Owner，具体缺陷按适用授权修复。
- **唯一执行规范**：[ROADMAP](../ROADMAP.md)；实验/结果/对照结论从此索引，避免第二份真相。

### 2026-09-08 Golden Path 优先收口（Owner 最新确认）

第一目标为工作台正常入口使用 P1＋已验证人物 A，完成真实生成、可靠关联和下载。P3、人物B、三种数字人来源及3+3配对后置为Repeatability/Experiment Matrix，不阻塞首条。继续同账号零积分/零生成校准，先确认报价、计费上限和回执机制；不得为取回执先付费，也不得以列表差或最新作品代替归属。

Owner 已进一步明确“最多两次”指最多两条完整视频，含每条手持图、固有图片数字人及视频创作全部收费。该数量用于worst-case预算准备；当前未授权任何付费生成，首次Run须在Preflight全部PASS后单独批准。P1将在提供后绑定本轮许可与口播，不能擅自用历史玩偶替代。既有设置/比例、权限、幂等、付费不明停止和证据要求保持；不新增平台或审批角色。

### 2026-09-09 完整官方 API 的条件性优先（Owner 确认）

Owner要求先核查手里有货是否可由开放接口组合完成，并明确：如果完整功能能用官方API实现，优先API Key鉴权，减少日常生产对重复网页登录的依赖。当前只确认该取舍偏好，不把网页可见接口当开放API，不保证模型/效果相同，不宣称API必然无失败。以正式输入输出、账户权限、费用、异步回执及真实结果验证决定最小接入；权限、素材绑定、防重、预算和作品关联继续复用既有核心。尚未授权API激活、新Provider、生成、发布或重写系统。

### D-038 实现语义：人工口播无需额外模型

Owner 已明确要求本轮直接输入中文且不强制额外模型。人工输入来源固定持久化，复用既有文案版本、QC 与人工审核核心。本地规则只证明实际执行的数字、直接矛盾与配置规则检查；未执行模型语义检查时用需 Owner 确认的 finding 明示，保留实际接受理由和 Review，不自动批准。旧 AI 文案策略与 invalid/blocked 硬拦截保留。候选仍需独立 Review 和 merge/deploy 授权，不能在生产中把测试替身当成真质检。

### D-038 实现语义：2026-09-08 Owner 确认设置与比例交付政策

Owner 在审阅上一轮具体提案后回复“确认授权”：配对设置采用播客-女声/普通话、字幕开启、智能适配、AI 自动文案关闭。生成前仍记录 9:16 目标、实际可控设置及未证明项，不再要求提前证明尚未生成的原片尺寸；生成后实测原片，交付片两组统一采用保持完整画面、无拉伸、无裁切的等比补边得到严格 9:16，保留原片及不匹配记录。声音依现有 `ui_visible_state_allowed` 按真实可见名称/风格核对，不新增不可见 voice_id 门槛。

仅新任务的明确政策可使用此语义，已持久化历史合同不得静默变更或被迁移成新政策。人物/商品/口播绑定、权限、费用上限、防重、提交不明停止和可靠作品关联不变；未知不是 PASS。不可解码、归属不明或交付处理失败停止且不重新付费。回滚恢复旧严格策略，同时保留所有输入、原片、交付片和证据。候选实现和 Review 在隔离分支推进，合并/部署仍需独立授权。

另批准一次最多 30 分钟、0 积分的账号/完整报价/作品关联校准，允许必要非付费设置临时调整并还原；素材上传、建任务、数字人创建/克隆、生成/再生成、视频下载、外部付费模型均为 0。页面访问/遥测/设置写入单列。计时自本轮首次真实校准访问开始，记录在现有 session；该授权不等于六条 R3 预算。

实施边界（主控技术 Review）：保留旧合同 canonical bytes/hash、输入绑定、严格付费门禁及空 verifier 的零浏览器保护。Worker 先完成配置/身份/存储检查、只读选择既有候选任务与交接包，再根据包内合同检查执行能力；空队列、旧合同缺 verifier 或非法合同不打开浏览器。新合同的真实登录/环境检查通过后才允许原有 claim 事务，任务运行中的早期与最终付费前字段读回继续强制执行。元数据读取不等于领取/生成；不新增队列或状态机。无任务直接打开浏览器的中间候选未通过 Review，未合并/部署，不作为已批准的旧合同语义变更。
