...
| 390px 如何保留对象、状态、阻断与下一步 | Shopify 直接支持响应式列结构；Frame.io 的同一审核上下文只提供组织参考 | **Inference / Adapt**：结合上述模式与本项目移动端内部审计，推导任务优先重排；**Reject** 单纯桌面单列化 | “首屏保留对象、状态、阻断和下一步”是 Hifly 设计推断，不是外部来源原话；筛选/技术证据是否进入抽屉仍待设计合同决定 | 各页移动首屏信息预算、列表/详情切换、sticky 动作边界和返回上下文 |
...
- **P1/P2：错误、权限不足和加载状态可能与内容同时出现。**
  - Adopt：Worklist 资格与快捷动作服从权限。
  - Inference / Adapt：结合权限模式与本项目“错误、loading 并存”的内部证据，推导错误态应替代陈旧内容，
    并给当前用户一个可执行恢复下一步；这是 Hifly 设计输入，不是外部来源的直接规范。
  - Reject：无权限时泄露对象存在性、加载失败后继续显示陈旧“可执行”、或用 toast 代替持久阻断。

### 5.4 响应式与素材

- **P1：390px 仅纵向堆叠，当前对象和下一步被推离首屏。**
  - Inference / Adapt：Shopify 只直接证明响应式列结构；“按任务优先级改变布局、不保持桌面模块完整顺序”
    来自该模式与本项目 390px 内部审计的组合推断。
  - Inference / Adapt：主列表/详情切换以及技术证据、历史、筛选进入抽屉或折叠，均须由后续 Hifly
    设计合同选择和验证，不能视为外部案例已经证明的方案。
...
5. **Inference / Reject：移动端完整复刻桌面。** 基于 Shopify 响应式结构与本项目内部审计，推断 390px
   应保留任务上下文并渐进披露，而不是把所有面板简单排成一列；具体排序仍由设计合同验收。
