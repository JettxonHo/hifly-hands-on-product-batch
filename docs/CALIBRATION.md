...
本节是一次有界 CUA 校准的观察，完整记录见 [FAST-MVP session](status/sessions/2026-09-07-fast-mvp.md)。没有上传新素材、点击生成或导出视频，不是正式 A/B 样本，也不证明新自动 verifier 已运行。

- **声音读回**：当前主页面实际结构为 `.page-goods .controls-panel .auto-voice-box .voice-details`，其中 `p.voice-name` 是名称、`p.voice-style` 是风格。公共声音选择有两层：声音条目后再选风格。观察到播客-女声/普通话切换到小镇姑娘/小镇姑娘 活泼，刷新后保留；已恢复原选择。这只能证明 UI 名称/风格，不等于内部 voice_id 或最终声音身份。
- **字幕读回**：`.card-header` 内 h2 为“字幕”，对应 `button[role=switch]` 具有 `aria-checked`；不得根据按钮颜色或全页面开关的索引猜测状态。
- **刷新会改状态**：本次刷新后 AI 自动文案与字幕均恢复开启。关闭 AI 后出现的是一段与本次任务无关的 42 字文本，来源未确认。每次任务必须显式填写和检查口播；早期 verifier 成功后若还有导航/重置，必须在最终准备完成后再次检查，才能点击付费按钮。
- **旧手持图仍残留**：外层上传区域空白不代表账号无历史图片。打开弹窗会恢复 H0 已生成的手持图；重新编辑显示旧人物/商品图。未操作生成时出现的结果不得绑定成新任务产物。
- **大小与费用观察**：智能适配/超大/大/中/小/超小可选，小与智能适配切换可读回；本次已还原智能适配。内层生成/再次生成显示 150 积分，这是日期限定的阶段一报价，不能作为整条视频或未来报价。未取得当前完整阶段二报价，也未观察到比例设置控件。
- **作品关联限制**：被检查的作品卡片外层只有 `.video-common-box`、`.box-main` 和 `.bottom .box-title/.box-time`，未取得可用任务 ID 或提交因果回执。不能据此断言全站没有 ID，也不能把名称/时间或列表新增项升级为提交回执。
- **noVNC 校准限制**：画面更新晚于本地操作回执，输入焦点变化后要先核对再输入；过快连发曾漏掉地址首字符。CLI 控制的字段准备与 CUA 操作不能混称为正式纯 A 组。结束时应通过进程状态验证 Chrome 已关闭，不能依据残留远程画面判断。

声音/字幕期望已由 Owner 于当日后续确认；真实回执和两段费用上限仍是 R1/R2 前置条件，缺失时继续失败关闭。

同日第二次校准补充：当前 `.controls-panel .action-footer p.cost-breakdown` 含“图片数字人消耗 350 积分”与“视频创作消耗积分”，提示入口为 `.tooltip-container i.info-icon`（“查看积分说明”）。未读取完整视频计费单位，不能用孤立分项补成总报价。桌面校准视口存在 footer 不可见问题，临时响应式视口可显示；恢复设备模式/缩放后再清理。实际输入和 remote noVNC 画面可能晚于工具回执，须使用更新后的截图确认。浏览器关闭后 login Node 仍可能存活；应核对进程并只清理本轮实例。
