...
## 2026-09-17 收敛执行顺序（Owner确认实施）

继续既有R1/R2，先完成一条P1＋人物A正式闭环，再进入R5体验简化；影刀仅为有条件候选。当前实施允许隔离修改、零Provider验证、独立Review及Git/GitHub/部署元数据只读核对；真实页面校准、上传、付费、合并和部署分别停在已有授权边界。

| 顺序 | 实际任务 | 验收与停止边界 |
| --- | --- | --- |
| 1 | 核对开发/远端/部署与输入；冻结旁支新增投入 | 不把旧GUI/历史素材权限/历史账号当当前生产证明 |
| 2 | 整理历史回执与报价证据，准备一次最多30分钟同账号校准 | 本轮尚未授权校准；不上传、创建任务、生成、试听或下载视频 |
| 3 | 有真实schema证据后实现同次提交解析与精确结果关联 | 现有Report回执持久链复用；缺证据时不得猜字段或生成取证 |
| 4 | 费用未知独立阻断、指定Order范围、持久阶段防重；区分工作台401/403 | 不将估价/配置数字/boolean当可信费用上界；账号读回仍须真实证据 |
| 5 | 相关测试、一次独立Review、Required CI、发布/回滚与Preflight | 成本/回执/账号缺件仍BLOCKED；不以准备完成宣称可发布生产 |
| 6 | Preflight全PASS且独立授权后，一条完整Golden Path | 手持图/视频各最多一次提交；固有全部费用纳入cap；不自动追加第二条 |
| 7–8 | 成功后提出并确认合并内容/费用确认的体验，再清理有证据的重复逻辑 | 不伪造QC/审核，不迁移历史任务，不重写状态机；未证实无用不删旧路径 |
| 9 | 仅工具具体受限或Owner选择可视化维护时评估影刀 | 先本地Mac零积分验证；不并建第二套生产平台 |

当前检查点和证据统一见[前置条件报告](status/REAL_USER_VALIDATION_PRECONDITION.md#2026-09-17-收敛实施检查点)，不是第二份Roadmap。先完成所有不依赖外部证据的工作，再集中提出具体下一授权请求。
