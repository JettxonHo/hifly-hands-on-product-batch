...
P1 UX Next  #252 公共数字人缩略图：离线 source seam + `avatar_image` 导入/原子绑定已实现（无 migration、无新增 endpoint）；LocalObjectStore 的 body+metadata 提交、损坏 partial 恢复、固定 `.metadata.v2.json` hard-link 单赢家与同进程/独立 Node 并发已定向验证；真实 Hifly thumbnail read/login 校准仍待单独授权与验收，不得以 fake fixture 代替
