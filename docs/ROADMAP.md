...
- **状态**：PARTIAL。人物自动取用、直接人工口播和真实本地检查、付费后不明状态/中止防重、刷新请求恢复已形成可审查候选；独立代码 Review APPROVED。2026-09-08 有界零积分校准取得声音/字幕实际 DOM 与刷新行为、智能适配和 150 积分手持图报价；已复用同一校验回调补充最终准备后的复核，并保持付费前失败的持久状态。完整真实设置 verifier、冻结期望、因果回执与两段 quote/cap 尚未闭合，B 默认在上传前停止。当前全检/CI 真值见本轮证据。

## R2 — 冻结三组配对实验

- **目标**：形成真实可执行、可比较、可报价的 6 条清单。
- **前置**：R1 的硬阻断解决；候选已通过 Review/必要测试；被测环境可用且版本有证据。
- **任务**：A 为纯 Computer Use 原生页面，B 为工作台正常入口；P1 简单商品+A、P2 复杂玩偶+A、P3 另商品+B。冻结同一对素材版本/口播/声音/大小/字幕/比例、配置、账号、缓存及中间产物策略、代码/构建/时间。纯 A 工具不可用就 BLOCKED。默认 A1→A2→A3→B1→B2→B3，组内串行不靠手动清状态掩盖残留。
- **验收**：六条无未定身份/设置；技术、内容、交付三套口径；原片/交付比例分开；如需严格 9:16，先约定两组一致无变形处理且保留原片。全部预算/上传范围/停止条件可一次审批。缺失值不能填历史值代替。
...
- **状态**：BLOCKED / DRAFT_NOT_FROZEN；一次经 Owner 批准的零积分页面校准已结束，但仍缺 P1/P3、人物 B、重用/上传权限、完整报价、回执及被测发布。仅手持图 150 积分已观察，不足以申请整阶段消费预算；不能把未部署候选冻结为被测版本。

## R3 — 新授权下 A/B 各 3 条

- **目标**：取得所有开始、失败、成功与未执行的真实记录。
- **前置**：R2 冻结；Owner 批准实验/账号/环境/素材/上传目的地与范围、逐条和总积分上限、窗口/查询限制/停止规则；满足环境即时确认要求。
- **任务**：最多 6 视频；每样本手持图和视频各最多 1 次付费提交；重试/补拍/换路/额外样片 0。记录身份/版本、分段耗时、正常确认/CUA/开发者介入、提交/扣费/退款/净扣费、原片/交付片/归属/质量/错误和证据。
- **验收**：6 计划条目的事实完整（未执行也保留）；费用优先账单，否则说明余额归因限制；未知为 null；一次性建设成本另列；不把 Agent 点击计成人工劳动时长。
- **成本边界**：严格遵守新批准的逐条及剩余总预算；飞影之外成本单列不混加；没有报价或可能超限就不提交。
- **停止条件**：素材/账号/口播/设置错配、受理或扣费不明、重复风险、安全/登录问题、B 救场、影响可比性的版本/页面变化或技术失败停整阶段。普通画质不佳记录后可继续未受影响样本，不重做。
- **交付物**：追加 JSONL、私有原始证据与视频；修复前失败不可覆盖，新方案用新版本。
- **状态**：NOT_EXECUTED；预算 NOT_AUTHORIZED。

## R4 — 证据驱动取舍

- **目标**：决定实际需要多少自己的系统，不预选架构。
- **前置**：R3 已完成或按规则终止，所有样本有真实状态。
- **任务**：先逐条后汇总；区分计划/开始/完成/无介入/技术成功/可交付/失败/未执行。解释持续介入、换素材、关联、防重费用、飞影等待之外耗时；原因分工作台/飞影/环境/未知。比较保留现有、薄工作台、简洁默认+高级三候选。
