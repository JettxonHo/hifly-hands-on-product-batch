...
- Hifly Stage 1 结果在 UI Confirm 前读取 natural dimensions，严格使用
  ...
  历史真实运行当时明确要求 exact，因此其停止事实保持不变；正式
  ProductionOrder snapshot 当前固定写入 `record_only`，`require_exact` 只是
  deferred explicit execution-authorization seam，不是当前订单可选策略。Pre-paid
  mismatch 在 paid action 1 前停止；post-handheld mismatch 发生在 paid action 1
  后，只有显式 exact policy 才阻断 Confirm/action 2。final-video ratio 仍为
  `NOT_PROVEN`。当前 generated-result semantic selector 仍为 synthetic/unverified，
  `LIVE_DOM_EVIDENCE_REQUIRED`；商品保真只对消费者可见的重大身份/形状/颜色/腐化/替换/矛盾硬阻断；不可读小字不启用 OCR，记录为 `NOT_OBSERVABLE`/`NOT_REQUIRED`。
- Cloud 与 Local real V1 路径共享 structured verifier 检查；bare boolean、缺失、字段不全、上下文/期望值不匹配或未验证结果均在 `createAsset`/`submitVideo` 前停止，并投影受控结构化 evidence。`ui_input_copy_match` 与 `final_audio_copy_correspondence`、UI-visible voice 与 voice exact identity 分开记录；后者的 `voice_id`/`tts_voice_id`/`provider_voice_name`/`submitted_voice_name` 等未知值保持 `NOT_CAPTURED`/`NOT_PROVEN`，不得由 UI 展示推断。
- TDD/验证记录见 [`2026-09-01-issue-278-hifly-hands-on-product-v1.md`](sessions/2026-09-01-issue-278-hifly-hands-on-product-v1.md)；本阶段无 Provider/Hifly、真实登录、业务对象、部署或积分动作。新 run 的默认 evidence status 为 `hands_on_product/avatar/product=NOT_PROVEN`、`ui_visible_voice=NOT_CAPTURED`；历史基线单独保留为 hands_on_product/avatar/product/ui-visible-voice=`PROVEN/PARTIAL/PARTIAL`，并保留其 ui copy/ratio facts。
...
- Contract 固定 `mode=hands_on_product`、`target_aspect_ratio=9:16`、正式 ProductionOrder 当前固定 `handheld_aspect_ratio_policy=record_only`、`voice_source=hifly_native`、`voice_identity_policy=ui_visible_state_allowed`、`single_scene/fixed_simple/smart_fit` 与 no-B-roll/no-additional-characters/no-external-TTS/no-standalone-lipsync/no-copy-AI；`require_exact` 仅保留为 deferred explicit execution-authorization seam；不解析 `output_instructions`，不建设通用 DSL。Avatar server-private seam 只解析 current selection → AvatarVersion → registered MaterialVersion，并返回已核验 media/size/SHA，不加入公共投影；显式 contract + exact material checksum 才是 routing gate，不依赖通用 capability 名称。
...
- 本轮 TDD focused GREEN：contract 20/20、avatar material 8/8、evidence helper 9/9、execution snapshot 5/5、Local compiler 7/7、manual handoff service 13/13、real-chain 1/1、batch/Hifly page 95/95、Cloud Playwright 13/13、Cloud runtime 25/25、Cloud login 10/10、Local runner 20/20、Local execution system 3/3、production-start 15/15；这些 per-file 计数存在重叠，不相加；当前 bounded relevant aggregate 为 235/235。`npm run check` 检查 252 JS 文件，`git diff --check` 通过。完整 RED/GREEN 与命令见 [`2026-09-01-issue-278-hifly-hands-on-product-v1.md`](sessions/2026-09-01-issue-278-hifly-hands-on-product-v1.md)。
