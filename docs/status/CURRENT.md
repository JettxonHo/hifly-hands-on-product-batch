...
- **CORE-004（进行中，PR 待审查）**：portable-path API 边界加固（Issue #33）。
  ...
  - **person pool 契约**（`person-pool.js` 与 `person-strategy.js` 语义一致）：配置 rootDir 先经 `resolveFromRoot` 解析为 absolute filesystem path；持久化路径始终从项目根（`config.__rootDir`）到 absolute file 重新计算（`relativePortablePath(projectRoot, absoluteFile)`），配置值本身不直接持久化。**relative rootDir 支持；absolute rootDir 位于项目根内支持，且与 relative 形式产生完全相同的 persisted POSIX 路径；absolute/traversal rootDir 越出项目根 fail-closed，在 `readdirSync` 前拒绝、不读取外部目录**；缺 absolute `__rootDir` 的配置抛稳定边界错误。
  - **Hifly 下载**：`downloadArtifact` 在 `saveAs` 之前计算并验证 relative path（先验证后副作用）；越界 destination 在创建文件前抛错（saveAs 零调用、外部目录零写入），合法下载的文件名与 persisted relative path 不变。
