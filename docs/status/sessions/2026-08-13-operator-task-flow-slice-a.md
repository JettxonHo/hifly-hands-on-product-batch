...
- 目标：实现 Entry seam、Login、Projects、Project 和严格 opt-in 的运营任务流 UX，不改后端、API、数据库、
  身份授权、状态机或生产执行语义。
...
没有修改 `src/`、API、数据库/migrations、package/lock、Cloud Executor、Local Agent 或 Hifly 实现。

## 验证

- `node --test test/operator-task-flow-slice-a-browser.test.js test/project-content-browser.test.js`：3/3 通过。
- `node --test test/operator-task-flow-slice-a-browser.test.js test/gui-smoke.test.js test/frontend-foundation-browser.test.js test/identity-browser.test.js test/project-content-browser.test.js`：21 通过，1 跳过；跳过项是 identity browser 自身的可选环境门禁。
- Project API/service 聚焦回归：13/13 通过。
- `npm run check`：通过，检查 229 个 JavaScript 文件。
- `npm test`：1019 tests，971 通过，48 跳过，0 失败；其中 35 项因该次全量命令的 Chromium 沙箱环境门禁跳过，
