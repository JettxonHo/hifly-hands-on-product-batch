...
1. 允许并要求处理会直接影响核心功能、凭据安全、权限隔离或真实数据安全的风险，但禁止过度防御。
2. 除重大安全隐患外，不要新增哈希或 SHA-256；不得把哈希扩展到普通业务字段、非敏感数据或仅为形式完整的校验。
3. 密码不得明文存储、会话 Bearer Token 不得明文落库，属于身份系统核心安全边界，可以使用标准密码哈希与不可逆 Token 摘要。
4. 不要围绕基本不可能出现、没有真实触发路径或没有业务后果的 case 反复堆叠防御代码和测试。
5. Rubric、验收清单与安全审查用于发现真实风险，不得机械套用；优先考虑触发概率、业务后果、实现复杂度与维护成本。
6. 对非核心低概率风险，优先记录清晰的已知限制或后续项，不要阻塞当前业务闭环。
