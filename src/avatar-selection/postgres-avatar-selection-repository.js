  ...
  return {
    ...
    async listCatalog(organizationId) {
      const rows = (await pool.query(`SELECT a.*,v.id version_id,v.version_number,v.status version_status,v.authorization_status,v.authorization_expires_at,
        v.authorization_scope,v.capability_status,v.materials_accessible,v.preview_kind,v.created_at version_created_at,v.updated_at version_updated_at
      ...
      return Promise.all(rows.map(async (row) => ({
        asset: { id: row.id, organization_id: row.organization_id, source_type: row.source_type, display_name: row.display_name,
          description: row.description, status: row.status, controlled_seed: row.controlled_seed, seed_key: row.seed_key,
          seed_label: row.seed_label, created_at: iso(row.created_at), updated_at: iso(row.updated_at) },
        asset_version: normalizeVersion({ id: row.version_id, asset_id: row.id, organization_id: row.organization_id,
          ...
          capability_status: row.capability_status, materials_accessible: row.materials_accessible, preview_kind: row.preview_kind,
