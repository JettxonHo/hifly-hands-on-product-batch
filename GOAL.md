...
- 当前阶段：Wave 2，VSA-A01 已合并；VSA-A03 / Issue #59 的 PR #73 已完成实现、主控验收、独立审查并通过 CI，等待合并授权；A02 等待 A03 合并后实施。
...
| 2 | A02 / #58 ‖ A03 / #59 | 商品权威快照 ‖ 素材上传核验 | A03 PR #73 待合并；A02 ready 依赖 A03 |
...
1. 重新核对 A02/A03 的共享接口、Organization ownership 与 Migration 边界，再分别发任务包。
2. 获得用户对 A02/A03 的独立授权后启动实现，不自动越级开始其他 Issue。
3. 每个里程碑结束时更新本文件与 `docs/status/CURRENT.md`，再进入下一波次。
